  ...
  chrome.windows.getAll({ populate: true }, function(windowList) {
    ...
    for(var i=0; i < windowList.length; i++) {
      for (var j=0; j < windowList[i].tabs.length; j++) {
        console.log(windowList[i].tabs[j].openerTabId);
    ...
    // Printing the whole linear array of tab objects h
    // Making an ID-to-Index Map
    const idMapping = data.reduce((acc, elem, index) => {
      acc[elem.id] = index;
      return acc;
    }, {});
    ...
    data.forEach(element => {
     ...
     // console.log(element.id, element.parentId, element.children);
