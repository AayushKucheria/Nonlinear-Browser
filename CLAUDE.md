...
| File | Role |
|---|---|
| `manifest.json` | MV3 manifest; service worker = `background.js`, side panel = `sidepanel.html`; `_execute_action` command for `Ctrl+Shift+Y`; `focus-pin-1/2/3` commands for `Ctrl+Shift+1/2/3` (global pin shortcuts; Chrome limit = 4 total); permissions include `bookmarks`, `processes`; `host_permissions` includes `file:///*` (required for Chrome to consider granting file URL access; user must still enable "Allow access to file URLs" toggle at `chrome://extensions`) |
